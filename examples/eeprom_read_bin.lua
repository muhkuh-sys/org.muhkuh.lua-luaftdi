...
local tParser = argparse('FTDI EEPROM dumper', 'Dump the contents of an FTDI EEPROM to a file.')
...
-- Get the buffer.
local strBuf = tEeprom:get_buf()
hexdump(strBuf)
local tFile = io.open(tArgs.strOutputFileName, 'wb')
tFile:write(strBuf)
tFile:close()
